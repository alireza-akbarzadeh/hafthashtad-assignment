...
const Label: React.ForwardRefExoticComponent<
  LabelProps & VariantProps<typeof labelVariants> & React.RefAttributes<HTMLLabelElement>
> = React.forwardRef(({ className, asChild, htmlFor, ...props }, ref) => {
  const Comp = asChild ? Slot : "label"
  const labelProps = htmlFor ? { htmlFor } : {}

  return <Comp ref={ref} className={cn(labelVariants(), className)} {...labelProps} {...props} />
})
